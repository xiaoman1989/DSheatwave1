...
setwd("D:/地湖所工作/2025 热浪小论文XMGS02/Fig_S2")

## 读取biomass.xlsx文件，根据您提供的列名设置
rawdata <- read_excel(
  "biomass.xlsx",
...
## ======================= 混合计算方式（修正版） =======================
# 1. 计算总生物量（直接对所有物种求和）
total_biomass_daily <- rawdata %>%
  group_by(Day, Tank, Treatment) %>%
  summarise(
    total_biomass = sum(biomass_mg_L, na.rm = TRUE),  # 总生物量
    .groups = "drop"
  )
...
phylum_summary <- rawdata %>%
  ...
  summarise(
    biomass_mean = mean(biomass_mg_L, na.rm = TRUE),  # 门内物种平均生物量
    biomass_sum = sum(biomass_mg_L, na.rm = TRUE),    # 门内物种生物量总和
...
phylum_data <- phylum_summary %>%
  left_join(total_biomass_daily, by = c("Day", "Tank", "Treatment")) %>%
  mutate(
    proportion = biomass_sum / total_biomass,                  # 使用门内物种总和计算比例
...
## ======================= 统计总生物量变化 =======================
# 使用真实的总生物量进行统计
total_biomass_stats <- total_biomass_daily %>%
  group_by(Tank, Treatment, Day) %>%
  summarise(
    total_biomass_mean = mean(total_biomass, na.rm = TRUE),
    total_biomass_sd = sd(total_biomass, na.rm = TRUE),
    .groups = "drop"
  )

# 2. 计算每个处理组的Day 0和Day 8的变化
biomass_change_stats <- total_biomass_stats %>%
  filter(Day %in% c(0, 8)) %>%
  pivot_wider(
    id_cols = c(Tank, Treatment),
    names_from = Day,
    values_from = c(total_biomass_mean, total_biomass_sd),
    names_sep = "_"
  ) %>%
  mutate(
    change_absolute = total_biomass_mean_8 - total_biomass_mean_0,
    change_percent = (total_biomass_mean_8 - total_biomass_mean_0) / total_biomass_mean_0 * 100,
    Day0_value = sprintf("%.3f ± %.3f", total_biomass_mean_0, total_biomass_sd_0),
    Day8_value = sprintf("%.3f ± %.3f", total_biomass_mean_8, total_biomass_sd_8),
    Absolute_change = sprintf("%.3f", change_absolute),
...
biomass_change_stats <- biomass_change_stats %>%
  mutate(
    Treatment_Group = treatment_labels[as.character(Tank)]
  )

# 处理组汇总统计
treatment_summary <- biomass_change_stats %>%
  ...
  mutate(
    Day0 = sprintf("%.3f ± %.3f", Day0_avg, Day0_sd),
    Day8 = sprintf("%.3f ± %.3f", Day8_avg, Day8_sd),
    Absolute_change = sprintf("%.3f", Change_avg),
...
phylum_proportion_stats <- phylum_data %>%
  ...
  summarise(
    ...
    biomass_mean = mean(biomass_mean, na.rm = TRUE),          # 生物量平均值
  ...
  pivot_wider(
    ...
    names_from = Day,
    values_from = c(proportion_mean, biomass_mean),
    names_sep = "_"
  ) %>%
  mutate(
    ...
    biomass_change = biomass_mean_8 - biomass_mean_0,
    biomass_change_percent = (biomass_mean_8 - biomass_mean_0) / biomass_mean_0 * 100,
    ...
    Day0_biomass = sprintf("%.3f", biomass_mean_0),
    Day8_biomass = sprintf("%.3f", biomass_mean_8),
    Biomass_change = sprintf("%+.3f", biomass_change),
    Biomass_change_pct = sprintf("%+.1f%%", biomass_change_percent)
  ) %>%
  select(
    Tank, Treatment, Latin_phylum, species_count,
    Day0_proportion, Day8_proportion, Proportion_change, Proportion_change_pct,
    Day0_biomass, Day8_biomass, Biomass_change, Biomass_change_pct
  )
...
phylum_treatment_summary <- phylum_proportion_stats %>%
  ...
  summarise(
    ...
    avg_Day0_biomass = mean(as.numeric(Day0_biomass), na.rm = TRUE),
    avg_Day8_biomass = mean(as.numeric(Day8_biomass), na.rm = TRUE),
    avg_biomass_change = mean(as.numeric(Biomass_change), na.rm = TRUE),
    .groups = "drop"
  ) %>%
  mutate(
    ...
    Day0_biomass = sprintf("%.3f", avg_Day0_biomass),
    Day8_biomass = sprintf("%.3f", avg_Day8_biomass),
    Biomass_change = sprintf("%+.3f", avg_biomass_change)
  ) %>%
  select(
    Treatment_Group, Latin_phylum, n_tanks, avg_species_count,
    Day0_proportion, Day8_proportion, Proportion_change,
    Day0_biomass, Day8_biomass, Biomass_change
  )
...
# 1. 总生物量变化统计
addWorksheet(wb, "Total_Biomass_Statistics")
writeData(wb, "Total_Biomass_Statistics", 
          biomass_change_stats %>% select(Tank, Treatment, Treatment_Group, 
                                          Day0_value, Day8_value, Absolute_change, Percent_change),
          startRow = 1, startCol = 1)
setColWidths(wb, "Total_Biomass_Statistics", cols = 1:7, widths = "auto")
...
# 3. 各时间点详细数据
addWorksheet(wb, "Daily_Biomass")
daily_data <- total_biomass_stats %>%
  mutate(
    Biomass = sprintf("%.3f ± %.3f", total_biomass_mean, total_biomass_sd)
  ) %>%
  select(Tank, Treatment, Day, Biomass) %>%
  pivot_wider(
    names_from = Day,
    values_from = Biomass
  )
writeData(wb, "Daily_Biomass", daily_data,
          startRow = 1, startCol = 1)
setColWidths(wb, "Daily_Biomass", cols = 1:11, widths = "auto")
...
# 保存Excel文件
excel_file <- file.path(output_folder, "Total_Biomass_Statistics.xlsx")
...
max_biomass <- max(phylum_data$total_biomass)
...
## ---------- PDF ----------
pdf_file <- file.path(output_folder, "Fig_biomass_phylum_8.pdf")
ggsave(
  pdf_file,
  plot  = p,
  width = 10,
...
image_write(
  img,
  path = file.path(output_folder, "Fig_biomass_phylum_8.jpg"),
  format = "jpg"
)
...
cat("\n=== 混合方法验证 ===\n")
cat("藻门比例计算：先按门求平均值\n")
cat("总生物量计算：直接对所有物种生物量求和\n")

# 显示示例数据验证
example_tank_day <- phylum_data %>%
  filter(Tank == 57, Day == 0) %>%
  select(Tank, Day, Latin_phylum, biomass_mean, total_biomass, proportion_percent) %>%
  head()
...
cat(sprintf("3. 图形(JPG)已保存到: %s\n", file.path(output_folder, "Fig_biomass_phylum_8.jpg")))
cat("\n注意：使用混合计算方法：\n")
cat("  - 藻门比例 = 门内物种平均生物量 / 总生物量\n")
cat("  - 总生物量 = 所有物种生物量直接求和\n")
cat("\nExcel文件中新增的工作表：\n")
